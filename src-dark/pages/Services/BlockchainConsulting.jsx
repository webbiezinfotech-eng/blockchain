      ...
      <section className="relative w-full bg-white pt-4 pb-2 transition-colors duration-300 dark:bg-black sm:pb-8 md:pt-6 md:pb-8 lg:pt-8 lg:pb-8 xl:pt-10 xl:pb-8">
        ...
        <div className="px-4 sm:px-8 md:px-16 lg:px-20 flex w-full flex-col justify-start gap-5 leading-[1.4]">
            <h1 className="font-heading font-bold uppercase tracking-tight transition-colors duration-300">
              <span className="block  leading-[1.5] text-brand-blue  sm:leading-[1.25] text-[32px] leading-[1.2] sm:text-[48px] lg:text-[72px] lg:leading-[1.35]">
                EXPERT BLOCKCHAIN
              </span>
              <span className="block  leading-[1.5] text-[32px] leading-[1.2] sm:text-[48px] lg:text-[72px]">
            ...
            <p className="max-w-[760px] font-sans text-base font-medium leading-[1.7] text-black/75 transition-colors duration-300 dark:text-white/75 sm:text-xl">
 ...
 <section className="w-full bg-white py-4 transition-colors duration-300 dark:bg-black sm:py-8">
  <div className="mx-auto max-w-layout px-6 sm:px-10 md:px-16 lg:px-[3rem]">
    ...
    <div className=" text-center">
      ...
      <p className="mx-auto mt-4 mb-4 max-w-[920px] font-sans text-[16px] sm:text-[18px] font-medium leading-[1.7] text-black/70 dark:text-white/75 ">
...
>
  {consultingApproach.map((c, idx) => (
    ...
    >
      <div className="relative flex flex-col items-center h-full min-h-[480px] sm:min-h-[520px]">
        ...
        <div className="relative z-10 flex flex-col items-center justify-between h-full px-6 py-10 sm:px-8 sm:py-12">
          ...
          {/* IMAGE */}
          <div className="rounded-xl bg-white p-4 shadow-sm dark:bg-black/40">
            <img
              src={c.image}
              alt={c.title}
          ...
          {/* TITLE */}
          <h3 className="mt-6 text-center font-heading text-[18px] sm:text-[20px] font-bold uppercase tracking-wide text-brand-blue dark:text-white">
            {c.title}
          </h3>

          {/* DESCRIPTION */}
          <p className="mt-3 text-center text-[14px] sm:text-[16px] leading-relaxed text-black/75 dark:text-white/85 flex-grow">
            {c.desc}
          </p>
    ...
    <div className="mt-8 flex items-center justify-center gap-6">
      ...
      {/* Areas of Expertise */}
      <section className="w-full bg-white py-4 transition-colors duration-300 dark:bg-black">
        <div className="mx-auto max-w-layout px-6 sm:px-10 md:px-16 lg:px-[3rem]">
          ...
          "
              >
                {/* LEFT CONTENT */}
                <div>
                  <h3 className="font-heading text-[24px] font-bold uppercase tracking-wide text-black transition-colors duration-300 dark:text-white">
                    {e.title}
                  </h3>
                  ...
                  <ul className="mt-4 space-y-3">
                    {e.bullets.map((b) => (
                      <li key={b} className="flex items-start gap-3">
                        ...
                        <span className="font-sans text-[15px] sm:text-[18px] font-medium leading-relaxed text-black/75 transition-colors duration-300 dark:text-white/80">
                          {b}
                        </span>
                      </li>
                    ))}
      ...
      <section className="w-full bg-white py-4 transition-colors duration-300 dark:bg-black sm:py-8">
        <div className="mx-auto max-w-layout px-6 sm:px-10 md:px-16 lg:px-[3rem]">
          ...
          <div className="mb-12 text-center">
            ...
            <p className="mx-auto mt-4 max-w-[980px] font-sans font-medium leading-[1.7] text-black/70 dark:text-white/75 text-[16px] sm:text-[18px]">
              ...
              {deliverableTabs.map((d) => (
                <div key={d.title} className="w-full shrink-0">
                  <div className="grid grid-cols-1 items-center gap-10 md:grid-cols-[320px_1fr]">
                    ...
                    {/* Content (RIGHT) */}
                    <div>
                      <h3 className="font-heading text-[24px] font-bold uppercase tracking-wide text-brand-blue dark:text-white">
                        {d.title}
                      </h3>
                      <p className="mt-2 font-sans text-[16px] sm:text-[20px] font-medium leading-relaxed text-black/75 dark:text-white/80">
                        {d.para}
                      </p>
                      <ul className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
                        {d.points.map((p) => (
                          <li key={p} className="flex items-start gap-3">
                            <span className="mt-[7px] h-2 w-2 rounded-full bg-brand-blue" />
                            <span className="font-sans text-[15px] sm:text-[18px] font-medium leading-relaxed text-black/75 dark:text-white/80">
                              {p}
                            </span>
                          </li>
                        ))}

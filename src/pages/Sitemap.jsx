...
const Sitemap = () => {
  ...
  return (
    <div className="w-full bg-white transition-colors duration-300">
      ...
      {/* Content Section */}
      <section className="w-full bg-white pb-14 transition-colors duration-300 sm:pb-16">
        <div className="mx-auto max-w-5xl px-6 sm:px-10 md:px-16 lg:px-[3rem]">
          <div className="space-y-8">
            ...
            <div className="rounded-xl border border-[#c8d7e4] bg-white p-6 shadow-[0_2px_10px_rgba(0,0,0,0.06)] transition-colors duration-300 sm:p-8">
              <h2 className="mb-6 font-heading text-2xl font-bold text-black transition-colors duration-300 sm:text-3xl">
                Main Pages
              </h2>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {mainPages.map((page) => (
                  <Link
                    key={page.path}
                    to={page.path}
                    className="rounded-lg border border-[#c8d7e4] bg-white px-4 py-3 text-center font-sans text-base font-medium text-black transition-all duration-300 hover:border-brand-blue hover:bg-brand-blue/10 hover:text-brand-blue"
                  >
                    {page.name}
                  </Link>
                ))}
      ...
      <BeforeFooter title="Need Help Finding Something?" buttonText="Contact Us" buttonLink='/contact'/>
    </div>
  );
};

...
{/* Next-Generation Blockchain and AI Solutions */}
            <p className="max-w-[760px] font-sans leading-[1.7] transition-colors duration-300 text-xl md:text-2xl">
              ...
              <span className="mt-3 block">
                ...
                <span className="font-semibold text-brand-blue">Security</span> —{' '}
                <span className="font-semibold text-brand-blue">Scalability</span>, ensuring enterprise-grade solutions for modern businesses.
              </span>
            </p>
          </div>
            ...
            <div className="mx-auto mt-12 flex max-w-[800px] flex-wrap justify-center gap-4">
              {categories.map((category) => (
                <button
                  ...
                  className={`rounded-xl border-2  font-heading text-[13px] sm:text-[14px] md:text-[15px] leading-[1.6] font-semibold uppercase transition-all duration-300 px-4 py-2 sm:px-6 sm:py-3 ${selectedCategory === category.id
                      ? 'border-brand-blue bg-brand-blue text-white shadow-lg'
                      : 'border-black/20 bg-white text-black hover:border-brand-blue/50'
                    }`}
                >
                  {category.name}
                </button>
          ...
          <div className="mx-auto max-w-layout px-6 sm:px-10 md:px-16 lg:px-[3rem]">
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filteredProducts.map((product) => (
                <div
                  key={product.id}
                  className="group flex flex-col overflow-hidden rounded-2xl border border-black/10 bg-white shadow-[0_8px_24px_rgba(0,0,0,0.12)] transition-all duration-300 hover:-translate-y-2 hover:border-brand-blue/50 hover:shadow-[0_16px_48px_rgba(0,0,0,0.18)]"
                >
                  {/* Product Image */}
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    <h3 className="font-heading text-[18px] sm:text-[20px] md:text-[22px] font-bold text-black transition-colors duration-300 ">
                      {product.title}
                    </h3>
                    <p className="mt-3 flex-1 text-[14px] sm:text-[15px] md:text-[16px] leading-relaxed leading-[1.6] leading-relaxed text-black/70 transition-colors duration-300 ">
                      {product.description}
                    </p>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {product.features.map((feature, idx) => (
                        <span
                          key={idx}
                          className="rounded-lg bg-brand-blue/10 px-3 py-1 text-[12px] sm:text-[13px] md:text-[14px] font-medium text-brand-blue"
                        >
                          {feature}
                        </span>
                      ))}
                    ...
                    {/* CTA Button */}
                    {/* <button className="mt-6 w-full rounded-lg border-2 border-brand-blue bg-transparent px-4 py-3 font-semibold text-brand-blue transition-all duration-300 hover:bg-brand-blue hover:text-white">
                    Learn More
                  </button> */}
                  </div>
                </div>

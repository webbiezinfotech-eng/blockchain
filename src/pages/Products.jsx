...
const Products = () => {
  ...
  return (
    <div className="w-full bg-white transition-colors duration-300 dark:bg-black">
        ...
        <div className="mx-auto grid w-full max-w-layout grid-cols-1 items-center gap-10 px-6 sm:px-10 md:px-16 lg:grid-cols-[58%_42%] lg:pl-[3rem] lg:pr-[5rem]">
          <div className="flex w-full flex-col gap-6">
            <h1 className="font-heading font-bold uppercase tracking-tight text-black transition-colors duration-300 dark:text-white">
              <span className="block text-[38px] leading-[1.2] sm:text-[52px] sm:leading-[1.50] lg:text-[52px] lg:leading-[1.35]">
                BLOCKCHAIN &amp; AI
              </span>
              <span className="block text-[38px] leading-[1.2] sm:text-[52px] sm:leading-[1.50] lg:text-[52px] lg:leading-[1.35]">
                PRODUCTS |{' '}
      ...
      {/* Category Filters */}
      <section className="w-full bg-white py-8 transition-colors duration-300 dark:bg-black sm:py-12">
        <div className="mx-auto max-w-layout px-6 sm:px-10 md:px-16 lg:px-[3rem]">
          <div className="text-center">
            ...
            <p className="mx-auto mt-3 max-w-2xl font-sans text-[15px] text-black/65 dark:text-white/70">
          ...
          <div className="mx-auto mt-12 flex max-w-[800px] flex-wrap justify-center gap-4">
            {categories.map((category) => (
              <button
                ...
                className={`rounded-xl border-2 px-6 py-3 font-heading text-sm font-semibold uppercase transition-all duration-300 sm:px-8 sm:py-4 sm:text-base ${
        ...
        <div className="mx-auto max-w-layout px-6 sm:px-10 md:px-16 lg:px-[3rem]">
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {filteredProducts.map((product) => (
              ...
              >
                {/* Product Image */}
                <div className="relative h-[240px] w-full overflow-hidden bg-gradient-to-br from-brand-blue/10 to-transparent">
                  <img
                    src={product.image}
                    alt={product.title}
                  ...
                  <div className="absolute bottom-4 left-4 right-4">
                    <span className="inline-block rounded-full bg-brand-blue/90 px-3 py-1 text-xs font-semibold uppercase text-white backdrop-blur-sm">
                      {product.category}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-1 flex-col p-6">
                  <h3 className="font-heading text-xl font-bold text-black transition-colors duration-300 dark:text-white sm:text-2xl">
                    {product.title}
                  </h3>
                  <p className="mt-3 flex-1 text-sm leading-relaxed text-black/70 transition-colors duration-300 dark:text-white/70 sm:text-base">
                    {product.description}
                  </p>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {product.features.map((feature, idx) => (
                      <span
                        key={idx}
                        className="rounded-lg bg-brand-blue/10 px-3 py-1 text-xs font-medium text-brand-blue dark:bg-brand-blue/20 dark:text-white"
                      >
                        {feature}
                      </span>
                    ))}
